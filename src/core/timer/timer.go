...
type LogicTimer interface {
	Resume(id string)
	Stop()
}

func (this *logicTimer) Stop() {
	this.endT = float64(time.Now().UnixNano()) / 1e6
	logs.Debug("%s used time:%.3fms", this.tag, this.endT-this.beginT)
}

func (this *logicTimer) Resume(id string) {
	if id != "" {
		this.tag = id
	}
	this.beginT = float64(time.Now().UnixNano()) / 1e6
}

func NewLogicTimer(id string) LogicTimer {
	return &logicTimer{
		tag:    id,
		beginT: float64(time.Now().UnixNano()) / 1e6,
	}
}
